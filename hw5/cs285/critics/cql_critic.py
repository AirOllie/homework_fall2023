...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        qa_tn_values = self.q_net_target(next_ob_no)

        if self.double_q:
            next_actions = torch.argmax(self.q_net(next_ob_no), dim=1)
            q_tn = torch.gather(qa_tn_values, 1, next_actions.unsqueeze(1)).squeeze(1)
        else:
            q_tn, _ = torch.max(qa_tn_values, dim=1)

        target = reward_n + self.gamma * q_tn * (1-terminal_n)
