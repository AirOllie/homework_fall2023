...
def run_training_loop(
    ...
    for itr in range(config["num_iters"]):
        ...
        if itr == 0:
            ...
            trajs, envsteps_this_batch = utils.sample_trajectories(env,random_policy,config["initial_batch_size"],config["ep_len"])
        else:
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                sample_dict = replay_buffer.sample(config["train_batch_size"])
                observations = sample_dict["observations"]
                actions = sample_dict["actions"]
                # rewards = sample_dict["rewards"]
                next_observations = sample_dict["next_observations"]
                # dones = sample_dict["dones"]
                step_losses.append(mb_agent.update(i,observations,actions,next_observations))
