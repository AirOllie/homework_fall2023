...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        # Compute loss
        loss: torch.Tensor = F.mse_loss(q_values, target_values)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        action_samples = action_distribution.rsample(torch.tensor([self.num_actor_samples]))
        entropy = -action_distribution.log_prob(action_samples).mean(dim=0)
        return entropy
    ...
    def update(
        ...
        if self.soft_target_update_rate!=None:
